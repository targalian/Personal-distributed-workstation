...
2. **集群规模效应已实测 (iter-66/67)**: iter-66 三机集群 (1 Secretary + 2 Worker + mock LLM) 真机实测 F3.1 自动扩缩容 + F3.3 PM 迁移全链路 17/17 (修复 9 个真实 bug A-I); iter-67 五节点集群 (1 Secretary + 4 Worker 单机多实例模拟) 实压 13/13 — 4 积压全部派发无滞留 (Bug J 水位门槛修正)、4 Worker 各承载 1 任务无重复派发、FIFO 顺序、5 任务并发、杀机 F3.3 回归 (Bug K pm_agents 落表); 真物理多机 (>2 台) 的大集群行为仍待实压。
