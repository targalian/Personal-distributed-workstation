...
2. **集群规模效应已三机实测 (iter-66)**: 三机集群 (1 Secretary + 2 Worker + mock LLM) 真机实测背书 — F3.1 自动扩缩容 (队列积压≥2 触发 → 精确派发空闲 Worker → 置 running) + F3.3 PM 迁移 (杀机 → 孤立检测 → 任务重置 pending → 迁移到存活 Worker) 全链路 17/17 断言通过; 过程中收敛节点间派发协议 (认证头/task_data/映射一致性/取消清理) 并修复 9 个真实 bug (A-I); 但 >2 节点的大集群规模 (>5 节点) 仍未实压, 扩容阈值的集群级表现留待更大规模验证。
