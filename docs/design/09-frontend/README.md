...
**项目蓝图工作台** (iter-88):
- 项目卡片显示「📘 蓝图」标记；项目详情首屏展示使命/目标/非目标/验收标准/
  约束、路线图与决策日志
- 「编辑蓝图」打开结构化表单：多行文本逐行解析目标类字段；路线图与决策
  使用 `字段 | 字段 | 字段` 行格式，保存时转换为对象数组
- 数据源: `GET/PUT /api/projects/{id}/blueprint`；保存成功后刷新项目列表
  并回到项目详情
...
| 日期 | 迭代 | 摘要 |
|---|---|---|
| 2026-09-07 | iter-88 | 项目详情新增蓝图工作台: 结构化编辑 charter/roadmap/decisions, 保存后列表与详情联动刷新 |
...
| 2026-09-08 | iter-90 | 交付自检展示: 任务卡片验收自检徽章 (通过/未全过) + 交付弹窗逐条标准判断 + task_delivered toast 未达成提示; UI-063 待浏览器实测 |
...
| 2026-09-08 | iter-91 | 影子开发面板 (UI-064) + 停滞告警面板 (UI-065): 新增 🕶️ 影子开发 Tab (桌面+移动 nav; 守护状态条/提交表单前端校验 空+超4000字拦截/队列历史表 run 状态徽章/详情弹窗 门禁+Agent+Diff 段/503 降级为守护未启动) + 运行时 Tab 🚨 停滞告警区 (守护状态+阈值信息/告警表/立即巡检 toast/WS 联动刷新), 后端契约零改动; UI-064/065 Playwright mock 三态 30/30 实测通过 (截图 temp_resault/ui064_*.png / ui065_*.png) |
| 2026-09-09 | iter-91 | UI 审计修复轮 (F1-F5/F7): onclick ID 插值 5 处 esc→escJs 防实体解码截断注入 + 影子未知状态徽章转义 + error_captured 恒真守卫修正 (改判 panel-runtime active) + 手动巡检后 4s 抑制 WS 停滞重复 toast + refreshAllState 按 active Tab 补刷 runtime/shadowdev + 移动端导航溢出横向滚动 (overflow-x:auto + min-width:52px + nowrap); F6 (shadow WS 实时事件) 转 [Quest→Codex]; 审计报告 docs/reference/ui-tree-audit.md, checklist UI-066~068 登记, Playwright+WS mock 19/19 实测通过 (截图 temp_resault/ui_audit_f5_mobile_nav.png) |
| 2026-09-09 | iter-98 | 影子开发实时状态 (F6 闭环, Quest→Codex 交回): 后端 `shadow_dev.py` 四处状态迁移广播 `shadow_run_update` (queued/running/终态/cancelled), 前端 `onStationEvent` 联动 — 仅停留影子 Tab 时才 `refreshShadowDev()` (沿用 iter-91 F3 的 active 判据避免恒真守卫), 终态弹 toast 而排队/执行中不打扰 (否则一次运行连弹三次); 真实 event_bus sink 实测 queued→running→GATES_FAILED 三事件到达, Node DOM 替身五场景实测, 12 变异全部致红 |
| 2026-09-10 | iter-101 | 任务记忆统计与成本/断点视图 (backlog UI-066, checklist UI-069~071): 记忆面板 by_type 行点击下钻 /api/task-memory/stats (统计卡+常见错误Top3, 再点收起, 刷新保持下钻态) + 任务详情弹窗异步补「成本预估」(预估Token/依据/置信度/历史样本/预算适配/落盘预估) 与「断点」两段 (无数据/非200静默隐藏) + 断点列表「断点恢复」按钮 (复用 iter-53 POST resume, 404 提示无快照), 后端零改动; Playwright mock 回归 19/19 PASS (含 XSS 注入/双态/零 pageerror) |
| 2026-09-10 | iter-101 | 团队与子 Agent 拓扑 (backlog UI-067, checklist UI-072): pm-tree 团队头点击折叠/展开 (折叠态 Set 存 window, WS 刷新后保持) + 成员计数徽章 + 成员点击弹 Agent 详情 (/api/agents/{id}: 状态/并发、主机、技能、工具、模型偏好、版本/心跳; 404 显式提示, 无 agent_id 兜底) + 弹窗异步段序号令牌守卫 (防过期响应覆写, 同模式回补 UI-066 两段), 后端零改动; Playwright mock 18/18 PASS (含单引号 agent_id escJs 实测/XSS 零执行/零 pageerror) |
| 2026-09-10 | iter-101 | 运行时 JSONL 执行统计面板 (backlog UI-068, checklist UI-073): 运行时 Tab 新增「⚡ 执行统计」区 GET /api/runtime/stats (独立窗口 1h/6h/24h/7d; 子任务成功率/LLM调用均延P99/Token入出/状态徽章/模型技能分布条形表/错误Top5; 空窗口提示), 与「按模型统计」标题分别标注数据源 (JSONL 追踪 vs SQLite 审计), refreshRuntime 联动重拉, 后端零改动; Playwright mock 12/12 PASS (含 hours 透传/XSS 零执行/零 pageerror)。至此 backlog P1 三项 (UI-066/067/068) 全部交付 |
| 2026-09-10 | iter-101 | 云存储同步状态卡 (backlog UI-069, checklist UI-074): 资源 Tab 新增「☁️ 云存储同步」区 GET /api/cloud-sync/status (启用/配置/守护三徽章、endpoint/bucket/前缀/本地路径、自动同步间隔、最近同步↑↓与错误数) + 「立即同步」「测试连接」按钮 (POST sync/test, 反馈 toast, errors 非空转警告) + 降级态 (无 endpoint) 徽章置灰/按钮禁用/config.yaml 指引 + WS cloud_sync 推送静默重拉 (不另弹 toast 防与 HTTP 响应双弹), 后端零改动; Playwright+/ws mock 13/13 PASS (含 endpoint XSS 零执行/降级双态/零 pageerror) |
