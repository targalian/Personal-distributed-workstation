...
## 更新摘要
**变更内容**
- 改进了模型回退处理逻辑，特别针对API密钥配置缺失的情况增强了错误报告
- 现在能够区分提供者解析失败和实际API调用失败，提供更明确的错误信息帮助用户诊断问题
- 增强了降级链重试机制的错误处理能力
...
**重大更新**：系统现在支持多提供商模型池集成，包括阿里云 Token Plan、DeepSeek、OpenAI、Anthropic 和通义千问等，通过智能路由算法实现最优模型选择和自动降级链重试。**新增功能**：改进了模型回退处理逻辑，特别针对API密钥配置缺失的情况增强了错误报告，现在能够区分提供者解析失败和实际API调用失败。
...
**更新**：系统现在改进了降级链重试机制的错误处理能力，特别针对API密钥配置缺失的情况提供了更明确的错误报告。

当首选模型调用失败时，系统会自动沿降级链重试，并提供详细的错误诊断信息：
...
Model1-->>Runtime : 调用失败
Runtime->>Runtime : 检查API Key配置
alt API Key未配置
Runtime-->>Client : 返回"API Key未配置"错误
else API Key已配置但调用失败
...
1. **提供者解析失败**：模型ID无法解析为有效的提供商配置
2. **API调用失败**：提供商配置有效但实际API调用失败
...
**更新**：系统现在提供了更明确的API密钥配置错误报告，帮助用户快速诊断问题。

当出现以下错误信息时，表示API密钥配置有问题：

- **"以下模型的 API Key 未配置"**：表示提供者解析成功但API密钥环境变量未设置
- **"未配置 LLM API Key"**：表示所有提供商的API密钥都未配置
...
- **错误日志**：记录异常情况和错误堆栈
- ****新增**：API密钥配置错误日志**：提供详细的API密钥配置问题诊断
...
9. ****新增**：改进的API密钥配置错误报告**：能够区分提供者解析失败和实际API调用失败，提供更明确的错误信息帮助用户诊断问题
