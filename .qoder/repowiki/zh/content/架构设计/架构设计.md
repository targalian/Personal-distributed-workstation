...
- Python 后端（lan_mesh 包）：提供 Master/Worker 节点、发现服务、数据库、协议与 API 路由，以及新增的分布式任务编排、模型路由器、项目管理和 MCP 工具网关
...
## 核心组件
- MasterController：中心控制节点，负责 UDP 广播发现、HTTP API、WebSocket 推送、数据库持久化、共享文件夹管理与任务编排
...
- 前端应用：React + Tauri，通过 invoke 调用后端命令，实现设备列表、共享资源、传输管理
...
- Master 节点作为控制中心，提供 Web UI、REST API、WebSocket 实时推送与数据库持久化
- Worker 节点作为执行单元，负责本机信息采集与共享文件夹，通过 HTTP 向 Master 注册与心跳
...
MasterController --> DiscoveryService : "组合"
MasterController --> Database : "依赖"
MasterController --> FastAPI : "创建"
MasterController --> Orchestrator : "组合"
MasterController --> ModelRouter : "组合"
MasterController --> ProjectManager : "组合"
MasterController --> MCPGateway : "组合"
```
...
- 职责
  ...
  - 通过 UDP 发现 Master，HTTP 注册与心跳，维护 Agent Card
  - 执行 Master 分发的子任务，支持多种技能类型的处理
- 关键流程
...
participant M as "MasterController"
W->>D : "启动 UDP 发现"
D-->>W : "on_device_seen(Master)"
...
### WebSocket 实时通信架构
- 连接管理
  - Master 提供 /ws 端点，接受 WebSocket 连接，维护客户端集合
...
participant S as "Master WebSocket(/ws)"
C->>S : "建立 WebSocket 连接"
S-->>C : "首次推送 hosts 列表"
loop "后台推送"
...
### 分布式任务编排系统
- **架构角色**
  - Orchestrator：任务编排器，在 Master 端运行，负责任务分解、Agent 匹配、子任务分发与结果聚合
...
### 模型路由器
- **架构角色**
  - ModelRouter：模型路由器，在 Master 节点运行，负责任务难度分级、加权评分和降级链管理
- **核心功能**
...
### 项目管理系统
- **架构角色**
  - ProjectManager：项目管理器，在 Master 节点运行，负责项目生命周期管理和预算控制
- **核心功能**
...
### MCP 工具网关
- **架构角色**
  - MCPGateway：MCP 工具网关，在 Master 节点运行，负责统一管理 MCP Server 和工具调用
...
- Python 依赖
  ...
  - requests：HTTP 客户端（Worker 向 Master 发起请求）
...
## 故障排查指南
- 启动失败
  - 检查端口占用：Master/Worker 端口与 Discovery 端口
  - 检查配置文件：config.yaml 是否正确，路径与权限
- 发现异常
  - 确认防火墙放行 UDP 45454；检查广播地址计算是否正确
  - 使用 /api/probe/{ip} 主动探测目标 IP
- 心跳失败
  - Worker 无法连接 Master：检查 Master IP/端口、网络连通性
  - 心跳超时：检查 Master 服务器负载与数据库写入性能
- WebSocket 断开
...
这些组件共同构成了一个完整的分布式个人 AI 工作站平台，适合中小规模局域网场景下的设备协同、资源共享和智能任务执行。架构决策在易用性、可维护性与性能之间取得平衡，为未来的功能扩展奠定了坚实基础。
