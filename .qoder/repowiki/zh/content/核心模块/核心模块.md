...
## 简介
本文件面向 Work Station 的核心模块，系统性梳理以下关键子系统：设备发现（基于 UDP 广播）、数据库持久化（SQLite）、任务编排（DAG 算法）、文件共享（分布式目录）、模型路由器（L1-L4 难度分级 + 加权评分）、项目管理（预算控制 + 预算护栏）、MCP 工具网关、Agent 运行时。**重大架构演进**：从传统的Secretary-Worker模式扩展为包含Station Director的新架构，实现"基础设施管理"和"项目管理"的分离。文档解释模块职责、实现原理、交互关系、配置项、API 接口与扩展点，并提供使用场景与排障建议，帮助开发者快速理解与扩展。

## 项目结构
- 入口程序负责解析命令行参数、加载配置并启动 Station/Secretary/Worker。
- 核心子系统位于 lan_mesh 包内，包含配置、自检、主机信息采集、协议定义、发现、数据库、共享文件夹、API 路由、Station Controller、Station Director、Worker 守护进程、任务编排与 DAG 管理、模型路由器、项目管理器、MCP 网关、Agent 运行时、工具注册表等模块。
...
- **工作站控制器模块（新增）**
  - 职责：管理 Station Director、提供 Web UI、支持 Secretary 激活/停用。
  - 关键类：StationController；实现基础设施管理与项目管理的分离。
...
## 架构总览
**重大架构演进**：系统采用"基础设施管理 + 项目管理"的双层架构：
- **Station Director（基础设施管理层）**：负责主机注册、评级、资源池管理、离线检测、舰队统计。
- **Secretary（项目管理层）**：负责任务编排、项目管理、模型路由、MCP 工具网关、Agent 管理。
- **Worker**：负责采集本机信息、共享文件夹、UDP 发现 Station/Secretary、HTTP 注册与心跳、执行子任务。
...
- **网络与 I/O**
  ...
  - **新增**：Station Director 与 Worker 之间的 HTTP 通信支持远程 Secretary 子进程管理。
- **数据库**
...
- **日志定位**
  ...
  - **新增**：Station Director 会记录主机入站/离线事件，便于故障排查。
  - **新增**：Secretary 激活/停用会在 WebSocket 中实时推送状态变更。
...
- [lan_mesh/station_controller.py:312-404](file://lan_mesh/station_controller.py#L312-L404)

## 结论
Work Station 的核心模块围绕"基础设施管理—项目管理—任务执行"形成三层架构闭环：**Station Director** 提供设备发现与网络状态，**Database** 实现可靠持久化，**Station Director** 以主机评级驱动资源池管理，**SharedFolderManager** 提供简单高效的文件共享。**新增的 Secretary 模块实现了智能的 L1-L4 难度分级与加权评分路由，项目管理器提供了完善的预算控制与隔离机制，MCP 工具网关统一了外部工具的管理和调用，Agent 运行时实现了 Worker 端的多技能任务执行。**模块间通过清晰的 API 与协议边界协作，具备良好的可扩展性与可维护性。**重大架构演进实现了基础设施管理与项目管理的分离，提高了系统的可维护性和可扩展性。
