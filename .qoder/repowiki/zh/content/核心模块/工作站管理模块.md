...
- 更新架构概览以反映新的分层设计
- 新增Web UI仪表盘和实时监控功能
- **新增技能管理增强**：添加SkillRegistry实例化和自动技能注册功能，在启动时创建技能注册表并执行自动扫描
...
**更新** 新增了Station Director架构，实现了更灵活的基础设施管理层，支持独立的Station模式和远程Secretary分配功能。**新增技能管理增强**，通过SkillRegistry实现中央技能注册表管理，支持自动扫描skills/目录中的技能文件，并提供权限分配和内容分发功能。
...
- [lan_mesh/station_api.py:1-714](file://lan_mesh/station_api.py#L1-L714)
...
- [lan_mesh/station_api.py:42-714](file://lan_mesh/station_api.py#L42-L714)
...
subgraph "数据访问层"
K[Database]
L[SharedFolderManager]
end
subgraph "基础设施层"
M[DiscoveryService]
N[HostInfoCollector]
O[Protocol Definitions]
end
A --> D
B --> D
C --> D
...
```mermaid
flowchart TD
SkillsAPI[技能管理API] --> ListSkills[GET /api/station/skills]
SkillsAPI --> ScanSkills[GET /api/station/skills/scan]
SkillsAPI --> DownloadPackage[GET /api/station/skills/download]
SkillsAPI --> RoleSkills[GET /api/station/skills/role/{role}]
SkillsAPI --> SkillDetail[GET /api/station/skills/{skill_id}]
SkillsAPI --> AssignSkill[POST /api/station/skills/{skill_id}/assign]
SkillsAPI --> RevokeSkill[DELETE /api/station/skills/{skill_id}/assign]
```

**图表来源**
- [lan_mesh/station_api.py:626-677](file://lan_mesh/station_api.py#L626-L677)
...
subgraph "系统模块"
K[station_controller.py] --> A
K --> C
K --> E
K --> G
K --> I
L[station_director.py] --> C
L --> E
L --> G
M[station_api.py] --> A
M --> C
...
R[host_info.py] --> E
R --> G
S[discovery.py] --> T[socket]
S --> U[json]
T --> V[网络套接字]
U --> W[JSON序列化]
X[skill_registry.py] --> C
X --> I
X --> Y[pathlib]
end
subgraph "第三方库"
Z[uvicorn] --> AA[ASGI服务器]
AB[requests] --> AC[HTTP客户端]
AD[python-multipart] --> AE[文件上传]
AF[markupsafe] --> AG[Jinja2模板]
end
A --> Z
AB --> AC
AD --> AE
...
工作站管理模块是一个功能完整、架构清晰的分布式系统。通过新增的Station Director架构，系统实现了更加灵活和强大的基础设施管理能力。**新增的技能管理增强**进一步提升了系统的智能化水平。
...
**更新** 新的架构设计使得系统更加模块化和可扩展，为未来的功能增强和性能优化奠定了良好的基础。**新增的技能管理功能**为系统的智能化应用提供了强有力的支持，通过标准化的技能文件格式和权限管理机制，实现了技能的统一管理和高效分发。