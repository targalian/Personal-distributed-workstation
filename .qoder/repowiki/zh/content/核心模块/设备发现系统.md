...
- 更新了 _on_device_seen() 方法实现，消除了被动等待 HTTP 注册的流程
- 增强了设备发现的实时性和用户体验
- 完善了自动注册与手动注册的协调机制
...
设备发现系统是一个基于 UDP 广播的局域网设备发现框架，支持 Master/Worker 架构。**最新更新** 系统实现了自动注册功能，通过重写 `_on_device_seen()` 方法支持在 UDP 发现时自动构造 HostInfo 并注册，消除了被动等待 HTTP 注册的流程，显著提高了设备发现的实时性和用户体验。
...
subgraph "应用层"
I[secretary.py<br/>Secretary 控制器]
J[worker.py<br/>Worker 代理]
K[api.py<br/>API 路由]
end
subgraph "前端集成"
L[discovery.rs<br/>Rust 发现服务]
M[protocol.rs<br/>Rust 协议定义]
end
subgraph "配置文件"
N[config.yaml<br/>配置文件]
O[main.py<br/>入口程序]
P[model_pool.example.yaml<br/>模型池配置]
end
A --> B
A --> C
I --> A
...
- [station_controller.py:1-538](file://lan_mesh/station_controller.py#L1-L538)
...
**新增** StationController 是独立的工作站控制器，实现了 UDP 自动注册功能。
...
系统采用 Master/Worker 架构，通过 UDP 广播实现设备发现和状态同步。**更新** 引入了 Secretary 节点作为中心控制器和 StationController 作为独立工作站管理器，新增了 UDP 自动注册功能，进一步增强了设备的即时发现和管理能力。
...
- [discovery.py:33-259](file://lan_mesh/discovery.py#L33-L259)
- [station_controller.py:71-538](file://lan_mesh/station_controller.py#L71-L538)
- [station_director.py:28-232](file://lan_mesh/station_director.py#L28-L232)
...
**更新** StationController 作为独立的工作站控制器，实现了 UDP 自动注册功能：

#### UDP 自动注册机制

**新增** StationController 的 `_on_device_seen()` 方法实现了 UDP 自动注册功能：
...
**图表来源**
- [station_controller.py:297-333](file://lan_mesh/station_controller.py#L297-L333)
...
**图表来源**
- [worker.py:47-97](file://lan_mesh/worker.py#L47-L97)
...
5. **自动注册优化**: UDP 自动注册避免了额外的 HTTP 往返延迟，提高了响应速度
...
5. **自动注册防重**: 通过数据库查询避免重复注册，减少不必要的内存占用
...
5. **自动注册调试**: 检查 `_on_device_seen()` 方法的执行日志
...
8. **自动注册**: **新增** UDP 自动注册功能，消除了被动等待 HTTP 注册的流程，显著提高了设备发现的实时性和用户体验

**更新** 系统通过实现 UDP 自动注册功能，大大简化了设备接入流程，用户无需手动配置或等待 HTTP 注册即可完成设备发现和管理，提升了整体用户体验和系统响应速度。
...
- [station_controller.py:442-538](file://lan_mesh/station_controller.py#L442-L538)