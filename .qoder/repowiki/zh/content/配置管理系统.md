...
## 更新摘要
**所做更改**
- 新增Python环境变量配置文件支持，通过python-dotenv依赖实现敏感配置的外部化管理
- 更新了.gitignore文件以排除敏感配置文件，同时保留了示例模板供参考使用
- 在应用启动时自动加载.env文件，支持API Key等敏感信息的安全管理
- 增强了模型池配置的环境变量支持，支持多种AI服务提供商的密钥管理
...
- 配置文件结构与 YAML 语法规范
- 参数覆盖与环境变量支持机制
- **新增** Python环境变量配置文件(.env)支持与敏感配置外部化管理
...
## 项目结构
本仓库包含两套配置体系：
- Python 后端（LAN Mesh）：基于 Pydantic 的强类型配置模型，支持 YAML 文件、命令行参数覆盖和 .env 环境变量文件
...
DOTENV["python-dotenv<br/>环境变量加载器"]
...
- **新增** 环境变量配置文件加载器：通过 python-dotenv 自动加载 .env 文件中的敏感配置
...
## 架构总览
下图展示配置加载与覆盖的总体流程，包括新增的环境变量配置文件支持：
...
CLI->>DotEnv : 启动时加载 .env 文件
DotEnv->>Env : 注入环境变量(API Key等)
...
- 环境变量
  ...
  - **新增** .env 文件支持：通过 python-dotenv 自动加载项目根目录下的 .env 文件
- 命令行覆盖
  - --config/-c：显式指定配置文件路径
...
- 覆盖优先级
  - 命令行参数 > YAML 配置 > 环境变量 > 默认值

**更新** 新增了.python-dotenv环境变量配置文件支持
...
- 当前能力
  ...
  - **新增** .env 文件在应用启动时自动加载，支持运行时环境变量注入
...
### .env 文件支持
系统现在支持通过 python-dotenv 库自动加载项目根目录下的 .env 文件，用于管理敏感配置信息：

#### 自动加载机制
- 应用启动时自动尝试加载 .env 文件
- 如果 python-dotenv 未安装，自动跳过加载过程
- 环境变量注入后，其他模块可直接通过 os.environ.get() 访问

#### 支持的敏感配置类型
- AI 提供商 API Key（DEEPSEEK_API_KEY、OPENAI_API_KEY、ANTHROPIC_API_KEY、QWEN_API_KEY）
- 阿里云 Token Plan API Key（ALIYUN_TOKENPLAN_API_KEY）
- 其他第三方服务凭证

#### 安全最佳实践
- **禁止** 将 .env 文件提交到版本控制系统
- 使用 .gitignore 规则排除敏感配置文件
- 提供 .env.example 模板文件供开发者参考
- 在生产环境中通过容器编排或系统环境变量注入

### 模型池环境变量支持
模型池配置通过 model_pool.yaml 文件管理，每个模型条目都定义了相应的环境变量名：

#### 环境变量映射
- DeepSeek 模型：DEEPSEEK_API_KEY
- OpenAI 模型：OPENAI_API_KEY  
- Anthropic 模型：ANTHROPIC_API_KEY
- 通义千问模型：QWEN_API_KEY
- 阿里云 Token Plan：ALIYUN_TOKENPLAN_API_KEY

#### 配置查找顺序
1. model_pool.yaml 精确匹配
2. PROVIDER_CONFIG 前缀匹配（兼容旧逻辑）
3. 环境变量读取

**更新** 新增了完整的环境变量配置文件支持和敏感配置管理机制

**章节来源**
- [main.py:22-28](file://main.py#L22-L28)
- [requirements.txt:9](file://requirements.txt#L9)
- [agent_runtime.py:315-366](file://lan_mesh/agent_runtime.py#L315-L366)
- [model_pool.example.yaml:1-331](file://lan_mesh/model_pool.example.yaml#L1-L331)
- [.gitignore:42-48](file://.gitignore#L42-L48)
...
- **新增** .env 文件相关问题
  - 确认 .env 文件位于项目根目录
  - 检查 python-dotenv 依赖是否正确安装
  - 验证环境变量名是否与代码中引用的一致
  - 检查 .gitignore 是否正确排除了 .env 文件

**更新** 新增了环境变量配置文件相关的故障排查指南
...
## 结论
本配置系统以 Pydantic 为核心，结合 YAML 文件、命令行参数和环境变量配置文件，提供了清晰、可扩展且易维护的配置管理方式。通过明确的加载顺序与覆盖规则，满足多场景部署需求。**新增的环境变量配置文件支持进一步增强了系统的安全性**，使得敏感配置能够安全地外部化管理。新增的 Bot Gateway 配置系统进一步增强了系统的实用性，实现了与移动端的无缝集成。建议在生产环境中配合环境变量配置文件与只读配置文件，提升安全性与稳定性。
...
- **新增** .env 文件支持：
  ...
  - ALIYUN_TOKENPLAN_API_KEY：阿里云 Token Plan API 密钥

**更新** 新增了.env文件支持的环境变量列表
...
ALIYUN_TOKENPLAN_API_KEY="your-aliyun-tokenplan-api-key-here"
...
**安全提示**：请勿将此文件提交到版本控制系统，项目已配置 .gitignore 自动排除此类敏感文件。
