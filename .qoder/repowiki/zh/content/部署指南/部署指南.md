...
## 更新摘要
**所做更改**
- 新增完整的阿里云百炼 Token Plan 配置支持，包括模型池配置和环境变量设置
- 更新环境初始化脚本，新增 ALIYUN_TOKENPLAN_API_KEY 的设置说明
- 新增 Token Plan 服务的配置指导和使用注意事项
...
**更新** 新增阿里云百炼 Token Plan 配置支持，提供完整的订阅制模型接入方案，包含环境变量设置、模型池配置和使用指导。

## 项目结构
Work Station 由 Python 后端与前端桌面应用两部分组成：
- Python 后端：LAN Mesh 核心，包含 Secretary/Worker 节点、API、发现服务、数据库与共享文件夹管理。
...
**章节来源**
- [main.py:19-90](file://main.py#L19-L90)
- [lan_mesh/secretary.py:68-332](file://lan_mesh/secretary.py#L68-L332)
- [lan_mesh/worker.py:62-325](file://lan_mesh/worker.py#L62-L325)
- [lan_mesh/api.py:103-526](file://lan_mesh/api.py#L103-L526)
- [lan_mesh/database.py:16-143](file://lan_mesh/database.py#L16-L143)
...
- API 层：统一的 FastAPI 路由，分别暴露 Worker 与 Secretary 的端点。
- 模型池：支持多供应商模型配置，包含成本、质量、速度评分和降级链。

**更新** 新增阿里云百炼 Token Plan 支持，提供订阅制模型接入方案，包含多品牌模型统一 Credits 计量。
...
## 架构总览
LAN Mesh 采用"Secretary/Worker"集中式控制与去中心化协作的混合架构：
...
- 端口范围：Secretary/Worker API 端口可配置，默认端口见配置文件。
...
## 性能考虑
- 端口选择：Secretary/Worker 默认端口可配置，避免冲突；发现 TTL 与心跳间隔影响延迟与资源消耗。
...
**更新** 新增模型路由器的性能考虑，包括加权评分算法的计算开销和降级链的容错机制，以及阿里云百炼 Token Plan 的订阅制计费模式对性能的影响。
...
- 注册/心跳异常：检查 Secretary 地址与端口是否正确、网络连通性、超时设置。
...
**更新** 新增阿里云百炼 Token Plan 相关的故障排查，包括 API Key 环境变量设置和 Token Plan 服务连接问题。

**章节来源**
- [lan_mesh/secretary.py:246-263](file://lan_mesh/secretary.py#L246-L263)
...
## 结论
Work Station 项目提供了清晰的 Secretary/Worker 架构与完善的局域网发现、注册、心跳与可视化能力。通过合理的网络规划、容器化与高可用部署，可在生产环境中稳定运行。建议结合本文的部署步骤、性能调优与安全加固建议，逐步完成上线与运维。

**更新** 新增阿里云百炼 Token Plan 配置支持，为用户提供更灵活的模型接入方案，特别是对于需要订阅制模型的企业用户。
...
- 配置文件：准备 config.yaml，设置发现端口、Secretary/Worker 端口、共享目录与设备名。
- 启动 Secretary：在控制节点运行，启动 Web UI 与 API。
- 启动 Worker：在各工作节点运行，自动发现并注册到 Secretary。
- 防火墙与网络：开放 UDP 45454 与 Secretary/Worker API 端口；确保子网广播可达。
...
- 端口映射：映射 Secretary/Worker API 端口与 UDP 45454（需要 hostNetwork 或 privileged 权限以支持广播）。
...
### Kubernetes 部署配置
- DaemonSet/Deployment：Secretary 建议 Deployment，Worker 建议 DaemonSet 或按节点数 Deployment。
...
- 服务发现：通过 ClusterIP 服务暴露 Secretary API，Worker 通过 UDP 广播发现 Secretary。
- 资源限制：为 Secretary/Worker 设置 CPU/内存限制，避免资源争用。
...
### 负载均衡与高可用
- Secretary 高可用：使用外部负载均衡器（如 HAProxy/Nginx/云 LB）分发到多个 Secretary 实例；注意会话粘性与状态一致性。
- Worker 节点：无状态，可水平扩展；通过共享文件夹与数据库实现状态共享。
- 数据库：SQLite 适合单实例；生产建议迁移到 PostgreSQL/MySQL 并配合主从或集群方案。
- 发现层：多 Secretary 场景下，Worker 仍通过 UDP 广播发现，Secretary 间通过数据库同步状态。
...
- 灾难场景：Secretary 节点故障时，切换到备用实例；Worker 重新注册与心跳恢复。
